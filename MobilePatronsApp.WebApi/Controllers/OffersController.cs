...
namespace MobilePatronsApp.WebApi.Controllers
{
	...
	public class OffersController : SecureApiController
	{
		...
		readonly Dictionary<int, Tuple<String, String>> errors = new Dictionary<int, Tuple<String, String>>
        {
			...
			{ ERROR_NO_DATA_AVAILABLE, new Tuple<string, string>("There are no offers available", "You do not currently have any offers available.") },
		...
		/// <returns></returns>
		[Route("list")]
		[ResponseType(typeof(WinStarSoap.offer[]))]
		public HttpResponseMessage Get()
		{
			var MOC = new WinStarSoap.MobileOffersClient();
			var resultsArray = MOC.getOffers(WinstarDataModel.UserName, WinstarDataModel.Password, WinstarDataModel.Facility, WinstarDataModel.IpAddress);
