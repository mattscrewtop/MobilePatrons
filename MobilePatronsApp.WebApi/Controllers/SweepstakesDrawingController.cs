...
namespace MobilePatronsApp.WebApi.Controllers
{
	...
	/// PATRON CONTROLLER
	/// </summary>
	[RoutePrefix("api/sweepstakes-drawings")]
	public class SweepstakesDrawingController : SecureApiController
	{
		...
		readonly Dictionary<int, Tuple<String, String>> errors = new Dictionary<int, Tuple<String, String>>
        {
			...
			{ ERROR_NO_DATA_AVAILABLE, new Tuple<string, string>("There are no drawings available", "There are no drawings available") },
		...
		/// <summary>
		/// LIST OF SWEEPSTAKES DRAWINGS
		/// </summary>
		/// <param name="dataModel"></param>
		/// <returns></returns>
		[Route("list")]
		[ResponseType(typeof(WinStarSoap.sweepDrawings[]))]
		public HttpResponseMessage Post(SweepstakesDrawingOffer dataModel)
		{
			var MOC = new WinStarSoap.MobileOffersClient();
			var resultsArray = MOC.getSweepDrawings(WinstarDataModel.UserName, WinstarDataModel.Password, WinstarDataModel.Facility, dataModel.Offer, WinstarDataModel.IpAddress);
